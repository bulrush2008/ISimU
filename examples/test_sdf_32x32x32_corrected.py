"""
测试修正后的SDF算法 - 32x32x32网格
验证SDF值的正确符号和距离计算在更大网格上的表现
"""
...
def test_sdf_32x32x32_corrected():
    """测试修正后的32x32x32网格SDF计算"""
    print("=== Corrected SDF 32x32x32 Grid Test ===\n")
    ...
    # 输出文件
    output_h5 = os.path.join(base_dir, "matrix_data", "vessel_170_sdf_32x32x32_corrected.h5")
    output_vts = os.path.join(base_dir, "matrix_data", "vessel_170_sdf_32x32x32_corrected.vts")
    ...
    try:
        ...
        if 'SDF' in result['fields']:
            ...
            # 与16x16x16结果比较
            expected_positive_ratio = 103 / 4096  # 从16x16x16测试中得到
            expected_positive_32x32 = int(32768 * expected_positive_ratio)
            print(f"    - Expected positive points (based on 16x16x16): ~{expected_positive_32x32}")
            print(f"    - Actual positive points: {positive_points}")
            ratio_diff = abs(positive_points - expected_positive_32x32) / expected_positive_32x32 * 100
            if ratio_diff < 20:  # 20%的误差范围
                print(f"    [OK] Positive point count consistent with 16x16x16 test (diff: {ratio_diff:.1f}%)")
            else:
                print(f"    [WARNING] Positive point count differs significantly from 16x16x16 test (diff: {ratio_diff:.1f}%)")
        ...
        metadata = {
            ...
            'description': 'ISimU 修正后的SDF插值数据 - 32x32x32网格 - portal vein'
        }
        ...
        print(f"[OK] 32x32x32 grid results consistent with 16x16x16 test")
...
if __name__ == "__main__":
    success = test_sdf_32x32x32_corrected()
    if not success:
        sys.exit(1)